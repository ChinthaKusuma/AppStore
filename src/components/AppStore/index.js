...
class AppStore extends Component {
  state = {select1: tabsList[0].tabId, searchInput: ''}
  ...
  render() {
    const {select1, searchInput} = this.state
    const list2 = appsList.filter(each => each.category === select1)
    ...
    return (
      <div className="bgContainer">
        <div className="bgContainer1">
          ...
          <ul className="ul2">
            {list3.map(each => (
              <AppItem item={each} key={each.tabId} />
            ))}
          </ul>
        </div>
      </div>
